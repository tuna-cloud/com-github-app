...
rem
rem Options
rem install                Install the service using server8x64 as service name.
...
if exist "%CATALINA_HOME%\bin\%SVREXE%.exe" goto okHome
rem CD to the upper dir
cd ..
set "CATALINA_HOME=%cd%"
:gotHome
if exist "%CATALINA_HOME%\bin\%SVREXE%.exe" goto okHome
echo The %SVREXE%.exe was not found...
...
set "EXECUTABLE=%CATALINA_HOME%\bin\%SVREXE%.exe"
...
set CLASSPATH=
for %%i in (%CATALINA_BASE%\libs\*.jar) do (
	call :concat %%i
)
